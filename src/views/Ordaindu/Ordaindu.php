...
<?php 
    ...
    if($_SERVER['REQUEST_METHOD']=="POST"){
        ...
        $txartelekoIzena = $_POST['txartelekoIzena'];
        $txartelZenbakia = $_POST['txartelZenbakia'];
        $epemuga = $_POST['epemuga'];
        $cvv = $_POST['cvv'];
        
    $sql = "INSERT INTO ordaindu (nan, herria, helbidea, herrialdea, postaKodea, txartelekoIzena, txartelZenbakia, epemuga, cvv)VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssssssss", $nan, $herria, $helbidea, $herrialdea, $postaKodea, $txartelekoIzena, $txartelZenbakia, $epemuga, $cvv);
    
    if ($stmt->execute()) {
        echo "Datuak ongi gorde dira";
    } else {
        echo "Akatsa datuak gordetzerakoan: " . $conn->error;
    }
...
function mostrarAlerta() {
    alert("Eskerrik asko erosteagatik");
}
</script>
<div class="container">
    ...
    <form action="">
        ...
        <br>
        <button onclick="mostrarAlerta()" type="submit" value="Ordaindu" class="submit-btn">Ordaindu</button>
</div>
</form>
